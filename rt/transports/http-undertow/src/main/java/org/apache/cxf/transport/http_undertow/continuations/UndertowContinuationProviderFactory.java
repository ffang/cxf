 ...
 * 
 */
public class UndertowContinuationProviderFactory implements ContinuationProviderFactory {

    final boolean disableUndertowContinuations 
        = Boolean.getBoolean("org.apache.cxf.transport.http_undertow.continuations.disable"); 
    
    public UndertowContinuationProviderFactory() {
    }
    
    public ContinuationProvider createContinuationProvider(Message inMessage, 
                                                           HttpServletRequest req,
                                                           HttpServletResponse resp) {
        if (!disableUndertowContinuations) {
            ServletRequest r2 = req;
            while (r2 instanceof ServletRequestWrapper) {
                r2 = ((ServletRequestWrapper)r2).getRequest();
            }
            if (!r2.getClass().getName().contains("undertow")) {
                return null;
            }
            ...
            try {
                ...
                if (((Boolean)o).booleanValue()) {
                    return new UndertowContinuationProvider(req, resp, inMessage);
                }
            } catch (Throwable t) {
                //ignore - either not a proper Undertow request object or classloader issue
                //or similar.
            }
        }
        return null;
