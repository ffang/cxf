 ...
 * is extra undertow configuration.
 */
public class EngineLifecycleTest extends Assert {
    ...
    public void setUpBus(boolean includeService) throws Exception {
        ...
        readBeans(new ClassPathResource("/org/apache/cxf/systest/http_undertow/cxf.xml"));
        readBeans(new ClassPathResource("META-INF/cxf/cxf.xml"));
        readBeans(new ClassPathResource("undertow-engine.xml", getClass()));
        if (includeService) {
    ...
    private void getTestHtml() throws Exception {
        ...
        FileInputStream htmlFile = 
            new FileInputStream("target/test-classes/org/apache/cxf/systest/http_undertow/test.html");    
    ...
    public void testUpDownWithServlets() throws Exception {        
        ...
        UndertowHTTPDestination jhd = (UndertowHTTPDestination) si.getDestination();
        UndertowHTTPServerEngine e = (UndertowHTTPServerEngine) jhd.getEngine();
        org.eclipse.undertow.server.Server undertowServer = e.getServer();

        Handler[] contexts = undertowServer.getChildHandlersByClass(WebAppContext.class);
